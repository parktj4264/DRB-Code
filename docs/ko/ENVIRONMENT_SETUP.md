...
사내 보안 프로그램이 새 패키지 폴더를 잠그면 설치 마지막의 파일 이동이 실패할 수
있습니다. DRB-Code의 setup은 이 문제를 피하도록 staging 이동 없이 프로젝트 라이브러리에
직접 설치합니다. 설치 중 오류가 나면 RStudio를 모두 닫았다가 다시 열고 setup을 한 번 더
실행합니다.

## 패키지 설치 시간과 재사용

처음 사용하는 **PC·Windows 사용자·R 버전(4.1 또는 4.5)** 조합에서는 lockfile의
패키지를 내려받으므로 수 분이 걸릴 수 있습니다. 설치가 끝난 패키지는 해당 Windows
사용자의 전역 `renv` cache에 보관됩니다. 이후 같은 PC에서 DRB-Code 폴더를 다른
위치로 복사해도 setup은 이 cache를 우선 재사용합니다. 원본 프로젝트의 환경은
사라지지 않고, 복사본은 별도 프로젝트 라이브러리를 가집니다.

다른 PC 또는 다른 Windows 사용자에게까지 cache를 공유하려면 사내 파일 서버에 공용
cache를 두는 별도 운영 설정이 필요합니다.
